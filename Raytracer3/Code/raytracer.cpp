...
Material* Raytracer::parseMaterial(const YAML::Node& node)
{
	...
	m->texture = new  Image(c);
        cout << "textura no nula " << m->texture << endl;
    }else{
	node["color"] >> m->color;
	m->texture = NULL;	
        cout << "textura nula " << m->texture << endl;
    }
...
bool Raytracer::readScene(const std::string& inputFilename)
{
    ...
    try {
	    ...
	    //Read the type of render mode needed
	       ...
	       if(renderMode=="gooch") {
		   const YAML::Node* sample = doc.FindValue("GoochParameters");
		   double b, y, alpha, beta;
		   (*sample)["b"] >>b;
		   (*sample)["y"] >>y;
		   (*sample)["alpha"] >>alpha;
		   (*sample)["beta"] >>beta;
		   Gooch *gooch= new Gooch(b,y,alpha,beta);
		   scene->setGooch(gooch);
	       }
